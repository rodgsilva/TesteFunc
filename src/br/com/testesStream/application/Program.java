...
public class Program {
	...
	public static void main(String[] args) {
		...
		double avg = list.stream()
					.filter(p -> p.getSexo().equals(sexo) )
					.map(p -> p.getIdade())
					.reduce((int) 0.0, (x,y)-> x + y) / list.size();
		...
		List<String> qtdPessoa18a25 = list.stream()	
		...
		Paciente pasMaisVelho = list.stream()
									.filter(p -> p.getSexo().equals("Fem"))
									.max(comp2)
									.get();
...
Comparator<Paciente> comp3 = Comparator.comparing( Paciente::getIdade );
		...
		Paciente pasMaisBaixa = list.stream()
									.filter(p -> p.getSexo().equals("Fem"))
									.min(comp3)
									.get();
